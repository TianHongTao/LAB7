...
int  LoadUserCallArray(FILE  *fp)
{
	...
	int item_num = 0;
	while (fscanf(fp, "%d,%d,%d", &usercall_list[item_num].user_floor, &usercall_list[item_num].user_target, &usercall_list[item_num].call_time))
		item_num++;
	//此处差计算usercall[item_num]的指令类型
...
void GetNextTimeStatus(ELEVATORSTATE  *elevator)
{
	...
	{
		while (elevator_a.serve_list->next_node->next_node==NULL)
		{
			index++;
		}
		while (elevator_b.serve_list->next_node->next_node == NULL)
		{
			index++;
		}
	...
	else
	{
		...
		while (p != NULL)
		{
			...
			else
			{
				SERVELISTNODE *a;
				SERVELISTNODE *q;
				a = (SERVELISTNODE *)malloc(sizeof(ResponseListNode));
				q = a;
				...
				if (f != m)
					...
					q->serve_state == 'E';
				while (elevator->serve_list->next_node == NULL)
				{
					elevator->serve_list = elevator->serve_list->next_node;
				}
				elevator->serve_list = q;
				status_change_flag = 1;
				if (p->next_node == NULL)
				{
					if (p == response_list->head)
...
void  ImportSimulateResult(FILE *fp)
{
	...
	char Rwait[MAX];
	while (response_list->head->next_node==NULL)
	{
